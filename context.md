...
**Why these two (not four)**: DDXPlus and MedQA were previously listed but dropped 2026-04-21. DDXPlus's 49-pathology respiratory set doesn't map to the seeded `clinical_general` chest-pain differential without additional content work; MedQA is MCQ medical knowledge, which tests the reader model, not the substrate architecture. Full rationale in `reasons.md`. The two remaining benchmarks directly exercise the substrate's load-bearing contributions: memory lifecycle (LongMemEval-S) and structured-claim-driven note generation (ACI-Bench).
...
**Reader and judge models** per benchmark: see `Eng_doc.md §3.5` for the full model-usage policy. **TL;DR — Opus 4.7 is NOT used as the eval reader**; primary reader is `Qwen2.5-14B-Instruct` (Apache-2.0, GCP-self-hosted via vLLM) with `gpt-4o-mini` / `gpt-4.1-mini` as secondary comparability readers matching published SOTA.
