...
class ManagedAgentOrchestrator:
    ...
    def __init__(
        ...
        self._handoff_agent_id: str | None = None
        self._bias_monitor_agent_id: str | None = None
        self._note_coauthor_agent_id: str | None = None
    ...
    def initialize(self) -> None:
        ...
        from src.agents.tools.handoff_tools import handoff_tool_manifest
        from src.agents.tools.bias_tools import bias_tool_manifest
        from src.agents.tools.note_coauthor_tools import note_coauthor_tool_manifest
        ...
        handoff = self.client.beta.agents.create(
            name="RobbyMD Shift Handoff Agent",
            model="claude-opus-4-7",
            system=_load_prompt("handoff_system.txt"),
            tools=handoff_tool_manifest(),
        )
        self._handoff_agent_id = handoff.id
        log.info("managed_agents.handoff_created", agent_id=handoff.id)

        bias_monitor = self.client.beta.agents.create(
            name="RobbyMD Diagnostic Bias Monitor",
            model="claude-opus-4-7",
            system=_load_prompt("bias_monitor_system.txt"),
            tools=bias_tool_manifest(),
        )
        self._bias_monitor_agent_id = bias_monitor.id
        log.info("managed_agents.bias_monitor_created", agent_id=bias_monitor.id)

        note_coauthor = self.client.beta.agents.create(
            name="RobbyMD Clinical Note Co-Author",
            model="claude-opus-4-7",
            system=_load_prompt("note_coauthor_system.txt"),
            tools=note_coauthor_tool_manifest(),
        )
        self._note_coauthor_agent_id = note_coauthor.id
        log.info("managed_agents.note_coauthor_created", agent_id=note_coauthor.id)
    ...
    def create_handoff_session(
        ...
        from src.agents.tools.handoff_tools import HandoffToolDispatcher

        session = self.client.beta.sessions.create(
            agent=self._handoff_agent_id,
            environment_id=self._environment_id,
            title=f"Shift handoff: {encounter_id}",
        )
        return ManagedAgentSession(
            self.client,
            session.id,
            HandoffToolDispatcher(),
            encounter_id,
        )

    def create_bias_monitor_session(self) -> "ManagedAgentSession":
        from src.agents.tools.bias_tools import BiasMonitorToolDispatcher

        session = self.client.beta.sessions.create(
            agent=self._bias_monitor_agent_id,
            environment_id=self._environment_id,
            title="Diagnostic bias monitor",
        )
        return ManagedAgentSession(
            self.client,
            session.id,
            BiasMonitorToolDispatcher(),
            "bias_monitor",
        )

    def create_note_coauthor_session(
        self, encounter_id: str
    ) -> "ManagedAgentSession":
        from src.agents.tools.note_coauthor_tools import NoteCoauthorToolDispatcher

        session = self.client.beta.sessions.create(
            agent=self._note_coauthor_agent_id,
            environment_id=self._environment_id,
            title=f"Note co-author: {encounter_id}",
        )
        return ManagedAgentSession(
            self.client,
            session.id,
            NoteCoauthorToolDispatcher(self.conn),
            encounter_id,
        )
...
class RawMessageOrchestrator:
    ...
    def create_handoff_session(self, encounter_id: str) -> "RawAgentSession":
        from src.agents.tools.handoff_tools import (
            HandoffToolDispatcher,
            handoff_tool_manifest,
        )

        return RawAgentSession(
            self.client,
            _load_prompt("handoff_system.txt"),
            _to_messages_format(handoff_tool_manifest()),
            HandoffToolDispatcher(),
            encounter_id,
        )

    def create_bias_monitor_session(self) -> "RawAgentSession":
        from src.agents.tools.bias_tools import (
            BiasMonitorToolDispatcher,
            bias_tool_manifest,
        )
        ...
        return RawAgentSession(
            self.client,
            _load_prompt("bias_monitor_system.txt"),
            _to_messages_format(bias_tool_manifest()),
            BiasMonitorToolDispatcher(),
            "bias_monitor",
        )

    def create_note_coauthor_session(self, encounter_id: str) -> "RawAgentSession":
        from src.agents.tools.note_coauthor_tools import (
            NoteCoauthorToolDispatcher,
            note_coauthor_tool_manifest,
        )

        return RawAgentSession(
            self.client,
            _load_prompt("note_coauthor_system.txt"),
            _to_messages_format(note_coauthor_tool_manifest()),
            NoteCoauthorToolDispatcher(self.conn),
            encounter_id,
        )
