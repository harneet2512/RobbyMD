...
_agent_sessions: dict[str, Any] = {}  # session_id -> agent session object
...
def _on_projection_updated(payload: dict[str, Any]) -> None:
    sid = payload.get("session_id", "demo")
    _broadcast(sid, serialize("ranking.updated", _build_ranking(sid)))
...
def _build_ranking(session_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        if _conn is None:
            return {"hypotheses": _default_hypotheses()}
        ...
        for c in claims:
            ...
            active.append(ActiveClaim(
                ...
                predicate_path=path,
                polarity=polarity,
                value_normalised=c.value_normalised,
            ))
        ...
        for bs in ranking.scores:
            count = sum(1 for a in active if any(
                ap.branch == bs.branch for ap in ranking.scores
                if any(al.claim_id == a.claim_id for al in bs.applied)
            ))
            hypotheses.append({
                "branch": bs.branch,
...
def _default_hypotheses() -> list[dict[str, Any]]:
    return [
        {"branch": "cardiac", "label": "ACS", "posterior": 0.25, "log_score": 0.0, "claim_count": 0},
        {"branch": "pulmonary", "label": "PE", "posterior": 0.25, "log_score": 0.0, "claim_count": 0},
        {"branch": "msk", "label": "MSK", "posterior": 0.25, "log_score": 0.0, "claim_count": 0},
        {"branch": "gi", "label": "GERD", "posterior": 0.25, "log_score": 0.0, "claim_count": 0},
    ]
...
async def lifespan(app: FastAPI):
    global _conn, _loop, _demo_thread
